/**
  Mitsubishi M80 post processor configuration for Huaya CNC
  Based on Autodesk Mitsubishi post (Revision 44227)

  Customized for:
  - Huaya CNC machining centre
  - Mitsubishi M80 controller
  - 4-axis: X, Y, Z + A (table rotation around X, +/-360 deg)
  - Indexing only (3+1), no simultaneous TCP, no G68.2 (option not licensed)
  - Manual tool length touch-off (operator sets H-offsets in tool table)

  PRE-FLIGHT CHECKLIST:
  1. Tool length offsets (H##) must be set manually in the controller
     tool table before running. The post outputs G43 H<toolnumber>.
  2. This post outputs direct A-axis positioning with M45/M46 brake
     control (M45 = unclamp, M46 = clamp). G68.2 inclined surface
     machining is NOT used because this Huaya/M80 does not have that
     option licensed.
  3. Test new programs in single-block mode with feedrate override low
     and rapid override low until verified.

  FORKID {35C5FB3B-83FE-4c62-BA0E-8FCE62E83D6C}
*/

description = "Huaya M80 (4-axis A indexing)";
longDescription = "Mitsubishi M80 milling post for Huaya CNC with 4th axis A (indexing only, no G68.2).";
vendor = "Twenty Three BVBA";
vendorUrl = "";
...
properties = {
  ...
  useTiltedWorkplane: {
    title      : "Use G68.2",
    description: "Enable to use G68.2 for 3+2 operations. LEAVE OFF on standard Huaya/M80, the inclined surface machining option is not licensed and G68.2 will be rejected by the controller. Only enable if your MTB has confirmed G68.2 is available.",
    group      : "multiAxis",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  toolChangeOrder: {
    title      : "Tool change block order",
    description: "Select either 'Txx M06' or 'M06 Txx'.",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Txx M06", id:"TxxM06"},
      {title:"M06 Txx", id:"M06Txx"}
    ],
    scope: "post"
  }
};
...
var diameterOffsetFormat = createFormat({prefix:"D", minDigitsLeft:2, decimals:0});
  ...
  if (isFirstCyclePoint()) {
    ...
    case "tapping":
      ...
      writeBlock(
        ...
        (getProperty("useRigidTapping") == "without") ? sOutput.format(spindleSpeed) : "",
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    if (getProperty("toolChangeOrder") == "M06Txx") {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    // 4th-axis brake ON (clamp A) on this Huaya:
    //   M45 = release brake (unclamp)
    //   M46 = apply brake   (clamp)
    // Uses modal fourthAxisClamp output: only emits when state changes,
    // suppressing redundant M46 calls within a single A-orientation
    // (the post engine calls LOCK from multiple code paths, this filters them).
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(46)); // CLAMP A-AXIS (modal)
  ...
  case COMMAND_UNLOCK_MULTI_AXIS:
    if (machineConfiguration.isMultiAxisConfiguration()) {
      writeBlock(fourthAxisClamp.format(45)); // UNCLAMP A-AXIS (modal)
      if (machineConfiguration.getNumberOfAxes() > 4) {
...
function defineMachine() {
  var useTCP = false; // indexing only, no TCP
  if (false) { // config komt van machine builder
    // A-axis: table-mounted, rotates around X, +/-360 deg
    var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-360, 360], preference:0, tcp:useTCP});
